...
type: project-doc
tags: [agenda-tjgo, banco-de-dados, mer, der, ddl, postgresql]
status: v0.1
criado: 2026-06-15
...
Quatro entidades de domínio + a **associativa `lotacao`** (nasce do relacionamento N:N — a entidade que não existe no modelo conceitual e passa a existir no lógico para resolver o "muitos-para-muitos").
...
> [!example] DML / CRUD — exemplos (versões completas estão no `dump.sql`)
> ```sql
