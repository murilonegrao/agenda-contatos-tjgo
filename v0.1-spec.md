...
| RF | Descrição | Story |
|----|-----------|-------|
| **RF01** | CRUD de **Unidade** (id, nome, tipo, endereço, responsável, criado_em); o cadastro exige ≥1 telefone e ≥1 e-mail (RN09) | US01 |
| **RF02** | CRUD de **Contato** (id, nome, cargo, criado_em); o cadastro exige ≥1 telefone e ≥1 e-mail (RN09) | US02 |
...
- **RN09** — Toda **unidade** e todo **contato** deve ter **pelo menos um telefone E pelo menos um e-mail** (cadastro contactável). É exigido pela aplicação no fluxo de cadastro (não por constraint do banco, pois é uma regra entre tabelas); o `dump.sql` segue a regra. Não conflita com RN02: a unidade pode ter telefones/e-mails e **zero contatos** lotados.
...
- [ ] **CA07** — O **arco exclusivo** é garantido: um telefone/e-mail tem **exatamente um** dono (unidade *ou* contato). O CHECK do banco e a validação do `service` rejeitam zero ou dois donos com mensagem clara (não stack trace cru); o console sempre amarra um único dono ao criar.
...
- [ ] **CA10** — Ao cadastrar uma unidade ou um contato, o sistema **exige pelo menos um telefone e um e-mail** antes de concluir (RN09); um telefone/e-mail inválido é recusado e pedido novamente, sem deixar o cadastro sem contato.
